...
Задания:
    ...
    3. Страница отправила во вьюху process_authorization_view данные, которые мы положили в переменную data.
       Распечатайте переменную data, чтобы посмотреть в каком формате хранятся входящие данные
...
def process_authorization_view(request):
    if request.method == 'POST':
        ...
        if data['username'] in USERNAME_TO_PASSWORD_MAPPER and \
            data['password'] == USERNAME_TO_PASSWORD_MAPPER[data['username']]:
